...
def create_logger(name: str):
    logger = logging.getLogger(f"JobQuest:{name}")
...
def create_session(
    *,
    ...
    When use_stealth=True and scrapling is available, returns a StealthSession
    backed by curl_cffi with Chrome TLS impersonation.
    :return: A session object
    """
    if use_stealth and SCRAPLING_AVAILABLE:
        return create_stealth_session(proxies=proxies, ca_cert=ca_cert)
...
def create_stealth_session(
    *,
    ...
    """
    Creates a Scrapling-backed stealth session with Chrome TLS fingerprinting.
    Falls back to create_session() if scrapling is not available.
    """
    if not SCRAPLING_AVAILABLE:
        return create_session(proxies=proxies, ca_cert=ca_cert, is_tls=True)
...
def set_logger_level(verbose: int):
    ...
    if level is not None:
        for logger_name in logging.root.manager.loggerDict:
            if logger_name.startswith("JobQuest:"):
